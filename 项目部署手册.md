...
5.  [启动与访问](#5-启动与访问)
    ...
    *   [5.2 本地生产模式](#52-本地生产模式)
    *   [5.3 服务器生产部署 (Nginx)](#53-服务器生产部署-nginx)
6.  [自动化任务](#6-自动化任务)
...
**目的**: 适用于本地开发、调试和快速预览。此模式下，前端代码会热重载，方便开发。
...
3.  **访问**: 启动成功后，通常会在终端显示类似 `http://localhost:5173` 的地址。在浏览器中打开此地址即可访问回测系统界面。

    > **提示**: 如果端口被占用，Vite 会自动尝试其他端口。请留意终端输出的实际访问地址。

### 5.2 本地生产模式

**目的**: 模拟生产环境，验证构建后的静态文件是否正常工作，但无需配置 Nginx 等 Web 服务器。这对于在本地验证最终部署效果非常有用。
...
1.  **构建前端应用**:
    ...
    pnpm build
    ```
    这会在 `web/dist` 目录下生成优化后的静态文件。

2.  **本地启动静态文件服务**:
    你可以使用任何简单的 HTTP 服务器来托管 `web/dist` 目录。这里推荐使用 `serve` 工具：
    ```bash
    # 如果未安装 serve，先全局安装
    npm install -g serve

    # 在 web 目录下，启动 serve 服务，指向 dist 目录
    serve -s dist
    ```

3.  **访问**: `serve` 启动后，通常会在终端显示 `http://localhost:3000` 或类似地址。在浏览器中打开此地址即可访问构建后的回测系统。

    > **注意**: `pnpm dev` 启动的是开发服务器，带有热重载等开发辅助功能；`pnpm build` 后通过 `serve` 启动的是纯静态文件服务，更接近生产环境。
