...
*   **核心逻辑**: 
    ...
    *   **除零错误修复**: 在 `calculate_trade_result` 函数中增加了对 `buy_price` 的有效性检查，避免了因 `buy_price` 为零导致的 `RuntimeWarning`。
...
4.  **生成报告 (并行化)**: 继续在 `py_file` 目录下，依次执行 `python full_backtest.py` 和 `python generate_stock_reports.py`。这两个脚本现在都支持并行计算，将自动利用多核 CPU 加速处理。
...
2.  **注册策略**: 打开 `py_file/full_backtest.py`，在 `strategies` 列表中加入你的新策略ID（例如 `new_strategy_buy`）。
