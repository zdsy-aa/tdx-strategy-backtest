...
graph TD
    subgraph A[Phase 1: 数据获取与存储]
        direction LR
        A1(mootdx/tdxpy API) -- 股票/指数日线行情 --> A2(a1_data_fetcher_mootdx.py)
...
| 脚本名称 | 功能作用 | 详细说明 |
| :--- | :--- | :--- || **a1_data_fetcher_mootdx.py** | **数据获取与管理** | 负责从 \`mootdx/tdxpy\` 获取股票和指数的日线数据，并存储为CSV文件。**已修复多进程数据串扰导致的代码与数据不一致问题。**支持 \`--full\` (全量)、\`--today\` (当日增量)、\`--date\` (日期范围覆盖) 三种模式。 | 2.3. 回测与分析脚本
