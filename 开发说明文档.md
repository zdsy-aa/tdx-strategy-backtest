...
graph TD
    ...
    subgraph "后端计算层 (Python)"
        ...
        CSV --> A2[a2_unified_backtest.py]
        CSV --> A5[a5_unified_analysis.py]
        A2 -->|回测结果| JSON1[strategies.json]
        A2 -->|信号数据| JSON2[signal_summary.json]
        A5 -->|分析报告| JSON3[stock_reports.json]
        A5 -->|预测数据| JSON4[forecast_summary.json]
    end
