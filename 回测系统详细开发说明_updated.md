...
graph LR
    ...
    B -->|生成/更新| C[data/day/*.csv<br/>原始日线数据]
    
    C -->|读取股票数据| D(indicators.py)
    D -->|计算技术指标| E[DataFrame<br/>含原始数据+各类指标]
    
    E -->|全市场策略回测| F(full_backtest.py)
    E -->|个股报告生成| G(generate_stock_reports.py)
    E -->|缠论5买点测试| H(105_chan_5buy_test.py)
    E -->|六脉神剑测试| F1(101_six_veins_test.py)
    E -->|基础买卖点测试| F2(102_buy_sell_points_test.py)
    E -->|基础卖点测试| F3(104_test_sell_points.py)
    E -->|稳健组合测试| F4(201_steady_combo_test.py)
    E -->|激进组合测试| F5(202_aggressive_combo_test.py)
    E -->|抄底策略测试| F6(bottom_fishing_strategy.py)
    
    F -->|生成| I[backtest_results.json<br/>策略总览]
    G -->|生成| J[stock_reports.json<br/>个股报告]
    H -->|生成| K[chan_backtest_results.json<br/>缠论买点报告]
    F1 -->|生成| I1[six_veins_results.json<br/>六脉神剑报告]
    F2 -->|生成| I2[buy_sell_points_results.json<br/>基础买卖点报告]
    F3 -->|生成| I3[sell_points_results.json<br/>基础卖点报告]
    F4 -->|生成| I4[steady_combo_results.json<br/>稳健组合报告]
    F5 -->|生成| I5[aggressive_combo_results.json<br/>激进组合报告]
    F6 -->|生成| I6[bottom_fishing_results.json<br/>抄底策略报告]
    
    L(update_web_data.py) -->|同步更新| I
    L -->|同步更新| J
    L -->|同步更新| K
    L -->|同步更新| M[strategies.json<br/>策略定义]
    
    X(auto_update_daily.py) -.->|每日自动化| B
    X -.->|每日自动化| F
    Y(fix_stock_names.py) -.->|修复名称| J
    Z(fix_backtest_json.py) -.->|修复JSON| I
    P(force_update_data.py) -.->|强制更新| B
    Q(fetch_stock_names.py) -.->|获取名称| B
    R(get_all_stocks.py) -.->|获取列表| B
    S(generate_web_data.py) -.->|生成数据| M
    T(data_fetcher.py) -.->|辅助获取| B
    
    I -->|策略总览数据| N[页面: Reports.tsx]
    J -->|个股报告数据| O[页面: ReportDetail.tsx]
    K -->|缠论买点数据| P1[页面: Indicators.tsx<br/>缠论]
    I1 -->|六脉神剑数据| P2[页面: Indicators.tsx<br/>六脉神剑]
    M -->|策略定义数据| Q1[页面: Strategies.tsx]
