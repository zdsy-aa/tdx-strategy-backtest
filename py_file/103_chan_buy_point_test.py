...
def process_single_stock(filepath: str) -> pd.DataFrame:
    try:
        ...
        results = []
        chan_signals = {'chan_buy1': '缠论一买', 'chan_buy2': '缠论二买', 'chan_buy3': '缠论三买'}
        
        for signal_col, signal_name in chan_signals.items():
...
def main():
    ...
    # 生成报告
    report_path = os.path.join(REPORT_DIR, 'chan_buy_points_all_stocks_report.md')
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("# 缠论买点全量股票回测报告\n\n")
    ...
    summary.to_csv(os.path.join(REPORT_DIR, 'chan_buy_points_all_stocks_summary.csv'), index=False, encoding='utf-8-sig')
