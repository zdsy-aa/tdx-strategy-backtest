...
SIGNAL_NAMES = {
    'chan_buy1': '缠论一买', 'chan_buy2': '缠论二买', 'chan_buy3': '缠论三买',
    'chan_strong_buy2': '缠论强二买', 'chan_like_buy2': '缠论类二买'
}
        ...
        for signal_col in CHAN_BUY_SIGNALS:
            for period in HOLD_PERIODS:
                ...
                if stats['trade_count'] > 0:
                    results.append({
                        'signal': SIGNAL_NAMES[signal_col],
                        'hold_period': period,
                        **stats
                    })
...
def main():
    ...
    summary = aggregate_results(all_results, ['signal', 'hold_period'])
    
    # 生成报告
    report_path = os.path.join(REPORT_DIR, 'chan_5buy_all_stocks_report.md')
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("# 缠论5个买点全量股票回测报告\n\n")
        f.write(f"**测试股票数量**: {len(all_results)}\n")
        f.write(f"**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("## 汇总统计 (平均胜率)\n\n")
        f.write("| 信号名称 | 持有周期 | 总信号数 | 总交易数 | 平均胜率 | 平均收益 |\n")
        f.write("| --- | --- | --- | --- | --- | --- |\n")
        for _, row in summary.sort_values('win_rate', ascending=False).iterrows():
            f.write(f"| {row['signal']} | {row['hold_period']}天 | {int(row['signal_count'])} | {int(row['trade_count'])} | {row['win_rate']:.2f}% | {row['avg_return']:.2f}% |\n")
            
    print(f"报告已生成: {report_path}")
    summary.to_csv(os.path.join(REPORT_DIR, 'chan_5buy_all_stocks_summary.csv'), index=False, encoding='utf-8-sig')
