...
def update_strategies_json():
    """
    读取单指标回测结果，仅同步到前端 strategies.json 中的 singleIndicatorStrategies 部分。
    优化点：移除对组合策略的更新，避免数据覆盖冲突。
    """
    # 1. 路径定义
    ...
    try:
        ...
        # 兼容性处理：如果 backtest_single.json 是元信息字典，则需要处理
        # 假设该文件是由 main 函数生成的包含 'results' 列表的字典，或者是直接的结果列表
        if isinstance(backtest_list, dict) and 'results' in backtest_list:
            backtest_list = backtest_list['results']
        elif isinstance(backtest_list, dict):
            # 如果只是元信息，则无法更新具体策略数据
            log("提示: backtest_single.json 仅包含元信息，跳过策略统计更新。", level="INFO")
            return

        # 转换为字典以便查找
        backtest_data = {item['id']: item for item in backtest_list}
    except Exception as e:
        log(f"读取回测结果失败: {e}", level="ERROR")
        return

    # 4. 读取前端 strategies.json
    try:
    ...
    # 5. 【核心修改】仅更新单指标策略 (singleIndicatorStrategies)
    updated_count = 0
    if 'singleIndicatorStrategies' in web_data:
        for strategy in web_data['singleIndicatorStrategies']:
            ...
            # ID 映射逻辑
            mapped_id = s_id
            if s_id == 'chan_lun_2buy': mapped_id = 'chan_buy2'
            if s_id == 'money_tree_buy': mapped_id = 'money_tree'
            ...
            if mapped_id in backtest_data:
                ...
                if stats:
                    strategy['stats']['total'] = {
                        ...
                        "avgReturn": stats.get('avg_return', 0),
                    ...
                    updated_count += 1
                    log(f"已更新单指标策略统计: {s_id}")

    # 6. 保存更新后的文件
    if updated_count > 0:
        with open(strategies_json_file, 'w', encoding='utf-8') as f:
            json.dump(web_data, f, ensure_ascii=False, indent=2)
        log(f"成功同步 {updated_count} 个单指标策略到 {strategies_json_file}")
    else:
        log("未发现匹配的单指标策略数据，未执行写入。", level="INFO")
