...
本指南适用于在 Linux (Ubuntu/Debian) 或 macOS 系统上部署和运行本项目。
...
# 安装 Python 3.11+, venv, Node.js, npm 和 Git
...
现在，您可以打开浏览器并访问 `http://localhost:5173` 来查看回测仪表盘。

### 10.3 常见问题 (Linux/macOS)

- **权限问题**: 如果在执行 `sudo` 命令时遇到问题，请确保您的用户具有管理员权限。
- **pip 镜像**: 如果 `pip install` 速度慢，可以切换到国内镜像：
  ```bash
  pip config set global.index-url https://pypi.tuna.tsinghua.edu.cn/simple
  ```
- **pnpm 镜像**: 如果 `pnpm install` 速度慢，可以切换到淘宝镜像：
  ```bash
